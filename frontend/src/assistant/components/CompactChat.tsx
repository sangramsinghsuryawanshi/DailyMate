...
export default function CompactChat({ conversationId, initialMessages = [], onOpenFull }: CompactChatProps) {
  ...
  function send() {
    ...
    const msg: Message = { id: Date.now().toString(), role: 'user', content: input.trim(), createdAt: new Date().toISOString() }
    setMessages((m) => [...m, msg])
    setInput('')
    setIsStreaming(true)
